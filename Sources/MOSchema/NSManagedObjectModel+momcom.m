...
- (instancetype)initWithSourceURL:(NSURL *)url error:(NSError **)error;
{
    NSString *ext = url.pathExtension;
    
    if ([ext isEqualTo:@"xcdatamodel"]) {
        NSURL *curl = [url URLByAppendingPathComponent:@"contents"];
        NSXMLDocument *xdoc = [[NSXMLDocument alloc] initWithContentsOfURL:curl options:nil error:error];
        return [[NSManagedObjectModel alloc] initWithXml:xdoc error:error];
    } else if ([ext isEqualTo:@"xcdatamodeld"]) {
        NSString *model_name = url.lastPathComponent.stringByDeletingPathExtension;
        NSURL *curl = [url URLByAppendingPathComponent: [model_name stringByAppendingString: @".xcdatamodel/contents"]];
        NSXMLDocument *xdoc = [[NSXMLDocument alloc] initWithContentsOfURL:curl options:nil error:error];
        return [[NSManagedObjectModel alloc] initWithXml:xdoc error:error];
    } else {
        if (error != nil) {
            *error = [NSError errorWithDomain:NSCocoaErrorDomain code:-1 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Unrecognized file: %@", url]}];
        }
        return nil;
    }
}
...
{
    self = [NSManagedObjectModel compileFromDocument:sourceModelDocument error: error];
    return self;
}
